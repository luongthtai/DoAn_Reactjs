    ...
    return (
        <section className='relative border-b hidden lg:block'>
            ...
            <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center'>
                ...
                <form onSubmit={handleSubmit} className='relative z-20 shadow-900' style={{ boxShadow: 'rgba(0,0,0,.05) 0 21px 36px' }}>
                    <input className='w-full px-5 py-3 rounded-lg outline-green-600 text-sm' placeholder='Search your products from here' />
                    <button className='absolute right-0 top-0 flex items-center h-full bg-emerald-600 gap-2 text-white px-7 rounded-r-lg' type='submit' onClick={handleSubmit}><FiSearch /> Search</button>
                </form>
            </div>
        </section>
    )
